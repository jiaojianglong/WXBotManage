...
class Msg():
    ...
    def __init__(self,msg):
        ...
        self.client_info = self.client_model.find_one(query_params={"_id":ObjectId(self.tcp_client)})
        if self.type_ == "connect_success":
            return
        self.wxbot = WXBot(self.wxclient, msg.botid)
        try:
            self.bot_info = self.bot_model.find_one(query_params={"wxid": msg.accept_wxid})
        except:
        ...
        # try:
        #     self.friend_info = self.friend_model.find_one(query_params={"wxid":msg.from_wxid})
        # except:
    ...
    def reply_at_text(self,text,at_wxid):
        self.wxclient.sendmassage.sendmsg("10|%s|%s|%s|%s|0" % (self.msg.botid, self.msg.from_wxid,at_wxid, text))
