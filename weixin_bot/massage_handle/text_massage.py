    ...
    def handle(self):
        ...
        if reply_pattern == 1:
            ...
            if self.msg.is_group:
                if not re.search(r'@' + self.bot_info.get("nickname", ""), self.text):
                    return
                else:
                    self.text = re.sub(r'@' + self.bot_info.get("nickname", ""), "", self.text)

            res = requests.post(url="http://192.168.11.24:8887/api/simple_qa", data={"content": self.text}).json()
