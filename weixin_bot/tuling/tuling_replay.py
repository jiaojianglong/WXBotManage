...
def tuling_replay(kwargs, msg):
    ...
    if kwargs.get("imageUrl", ""):
        body["perception"].update(
            dict(inputImage={"url": kwargs['imageUrl']}, ))
    if kwargs.get("mediaUrl", ""):
        body["perception"].update(
            dict(inputMedia={"url": kwargs['mediaUrl']}, ))
    ...
    if location.get("province", ""):
        body["perception"]['selfInfo']['location'].update(
            dict(province=location['province']))
    if location.get("city", ""):
        body["perception"]['selfInfo']['location'].update(
            dict(province=location['city']))
    if location.get("street", ""):
        body["perception"]['selfInfo']['location'].update(
            dict(province=location['street']))
