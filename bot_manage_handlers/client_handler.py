...
class ClientHandler(BaseHandler):
    ...
    def get(self, *args, **kwargs):
        ...
        collection_client_info = {tcp_client.id: {
            "connect_state": tcp_client.connect_state,
            "wxbot": tcp_client.client.get_weixin_num(),
            "name": tcp_client.name} for tcp_client in TCPClient.connect_list}

        if is_all:
            client_info = [{"client_id": client_id, "name": client.get("name")}
                           for client_id, client in
                           collection_client_info.items()]
            result['data'] = client_info
            return result
        query_params = {}
        if name:
            query_params.update({"name": name})
        all_client_info, page = self.client_model.search_read(
            page=page,
            page_size=page_size,
        ...
        for client in all_client_info:
            if client.get("_id") in collection_client_info.keys():
                client['connect_state'] = collection_client_info[
                    client.get("_id")].get("connect_state")
                client['wxbot'] = collection_client_info[
                    client.get("_id")].get("wxbot")
            else:
    ...
    def post(self):
        ...
        try:
            ...
            raise self.exception.CustomException("参数格式错误")
        if client_id:  # 更新操作
            self.client_model.update(
                {"name": name, "host": host, "accept_port": accept_port,
                 "send_port": send_port},
...
class ConnectClientHandler(BaseHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        if connect_state == "true":  # 创建连接
            client_info = self.client_model.find_one(
                query_params={"_id": ObjectId(client_id)})
...
class AutoFriendHandler(BaseHandler):
    ...
    def get(self, *args, **kwargs):
        result = self.init_parameter()
        client_id = self.get_argument("client_id")
        status_flag = True if self.get_argument("auto_friend_state",
                                                "") == "true" else False

        self.client_model.update({"auto_friend": status_flag},
                                 query_params={"_id": ObjectId(client_id)})
...
class AutoGroupHandler(BaseHandler):
    ...
    def get(self, *args, **kwargs):
        result = self.init_parameter()
        client_id = self.get_argument("client_id")
        status_flag = True if self.get_argument("auto_group_state",
                                                "") == "true" else False

        self.client_model.update({"auto_group": status_flag},
                                 query_params={"_id": ObjectId(client_id)})
